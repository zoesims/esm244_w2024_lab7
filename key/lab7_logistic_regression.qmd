...
Let's set up two competing models so we can compare using model selection methods.  One will be based on passenger sex and class, which logically, and from our exploration, seem like good predictors of survival vs. not survival.  The other model will be an essentially random model, based on passenger ID and the port they embarked from, which logically wouldn't seem to have much relationship to survival.

```{r}
f1 <- survived ~ sex + pclass
